...
// Thu Jan 27 08:09:23 2022
// Module under test: async_fifo
// Generated from original STIL file : "./patterns/occ_bypass/iddq/stil/async_fifo_TM4.stil"
...
`define TDATA_FILE "/home/rfic/Desktop/adapted/async_fifo/tmax/./max_sim/serial/occ_bypass/iddq/TM4/pat_serial_tb.dat"
`ifndef XTB_DUT_HIER 
...
`define MAXTB_HEADER " MAX TB Version O-2018.06-SP1\n Test Protocol File generated from original file \"./patterns/occ_bypass/iddq/stil/async_fifo_TM4.stil\"\n STIL file version: 1.0"
`define MAXTB_H_S "#############################################################################"
...
module /* maxtb */ async_fifo_test ();
   ...
   parameter WFT_IDS = 3, SIG_IDS = 64, SIG_IDS_W = 7, MAX_WIDTH = 39, MAX_SIGW = 1;
   ...
   reg [(8*NAMELENGTH-1):0]  WFTIDName [0:2];
   ...
   always begin: measure_sigs_p
    fork
     forever @(ALLOUTSIGS['d`\rdata[7] ]) begin
       #0 if (cur_stmt == `VectorStmt || cur_stmt == `load_unloadStmt || cur_stmt == `iddq_captureStmt ) measure_out_sig('d`\rdata[7] ,ALLOUTSIGS[39], cur_stmt); 
     end
     forever @(ALLOUTSIGS['d`\rdata[6] ]) begin
       #0 if (cur_stmt == `VectorStmt || cur_stmt == `load_unloadStmt || cur_stmt == `iddq_captureStmt ) measure_out_sig('d`\rdata[6] ,ALLOUTSIGS[40], cur_stmt); 
     end
     forever @(ALLOUTSIGS['d`\rdata[5] ]) begin
       #0 if (cur_stmt == `VectorStmt || cur_stmt == `load_unloadStmt || cur_stmt == `iddq_captureStmt ) measure_out_sig('d`\rdata[5] ,ALLOUTSIGS[41], cur_stmt); 
     end
     forever @(ALLOUTSIGS['d`\rdata[4] ]) begin
       #0 if (cur_stmt == `VectorStmt || cur_stmt == `load_unloadStmt || cur_stmt == `iddq_captureStmt ) measure_out_sig('d`\rdata[4] ,ALLOUTSIGS[42], cur_stmt); 
     end
     forever @(ALLOUTSIGS['d`\rdata[3] ]) begin
       #0 if (cur_stmt == `VectorStmt || cur_stmt == `load_unloadStmt || cur_stmt == `iddq_captureStmt ) measure_out_sig('d`\rdata[3] ,ALLOUTSIGS[43], cur_stmt); 
     end
     forever @(ALLOUTSIGS['d`\rdata[2] ]) begin
       #0 if (cur_stmt == `VectorStmt || cur_stmt == `load_unloadStmt || cur_stmt == `iddq_captureStmt ) measure_out_sig('d`\rdata[2] ,ALLOUTSIGS[44], cur_stmt); 
     end
     forever @(ALLOUTSIGS['d`\rdata[1] ]) begin
       #0 if (cur_stmt == `VectorStmt || cur_stmt == `load_unloadStmt || cur_stmt == `iddq_captureStmt ) measure_out_sig('d`\rdata[1] ,ALLOUTSIGS[45], cur_stmt); 
     end
     forever @(ALLOUTSIGS['d`\rdata[0] ]) begin
       #0 if (cur_stmt == `VectorStmt || cur_stmt == `load_unloadStmt || cur_stmt == `iddq_captureStmt ) measure_out_sig('d`\rdata[0] ,ALLOUTSIGS[46], cur_stmt); 
     end
     forever @(ALLOUTSIGS['d`full]) begin
       #0 if (cur_stmt == `VectorStmt || cur_stmt == `load_unloadStmt || cur_stmt == `iddq_captureStmt ) measure_out_sig('d`full,ALLOUTSIGS[47], cur_stmt); 
     end
     forever @(ALLOUTSIGS['d`empty]) begin
       #0 if (cur_stmt == `VectorStmt || cur_stmt == `load_unloadStmt || cur_stmt == `iddq_captureStmt ) measure_out_sig('d`empty,ALLOUTSIGS[48], cur_stmt); 
     end
     forever @(ALLOUTSIGS['d`near_full]) begin
       #0 if (cur_stmt == `VectorStmt || cur_stmt == `load_unloadStmt || cur_stmt == `iddq_captureStmt ) measure_out_sig('d`near_full,ALLOUTSIGS[49], cur_stmt); 
     end
     forever @(ALLOUTSIGS['d`near_empty]) begin
       #0 if (cur_stmt == `VectorStmt || cur_stmt == `load_unloadStmt || cur_stmt == `iddq_captureStmt ) measure_out_sig('d`near_empty,ALLOUTSIGS[50], cur_stmt); 
     end
     forever @(ALLOUTSIGS['d`over_flow]) begin
       #0 if (cur_stmt == `VectorStmt || cur_stmt == `load_unloadStmt || cur_stmt == `iddq_captureStmt ) measure_out_sig('d`over_flow,ALLOUTSIGS[51], cur_stmt); 
     end
     forever @(ALLOUTSIGS['d`under_flow]) begin
       #0 if (cur_stmt == `VectorStmt || cur_stmt == `load_unloadStmt || cur_stmt == `iddq_captureStmt ) measure_out_sig('d`under_flow,ALLOUTSIGS[52], cur_stmt); 
     end
     forever @(ALLOUTSIGS['d`test_so]) begin
       #0 if (cur_stmt == `VectorStmt || cur_stmt == `load_unloadStmt || cur_stmt == `iddq_captureStmt )
  ...
  endtask
   ...
   task iddq_capture ;
     ...
     input reg [2*CUM_WIDTH-1:0] valargs;
   begin
     if (verbose >= 2) $display("XTB: Starting proc iddq_capture..., T=%0t, V=%0d", $time, v_count+1);
     //check_sid_args("iddq_capture",64'b1, idargs);
     save_context;
     resolve_wft(2);

     assign_val('d`_po , 36'b111111111111111111XXXXXXXXXXXXXXXXXX);
   ...
   `ifdef tmax_iddq
    always @ iddq_strobe begin
  	 $ssi_iddq("strobe_try");
  	 $ssi_iddq("status drivers leaky async_fifo_test.leaky");
    end 
    `endif 
   ...
   always @(start_part) begin: TM4_occ_bypass_pattern__proc_unit
     ...
     if (diagm > 0) begin
       ...
       `else 
         diag_file = "./max_sim/serial/occ_bypass/iddq/TM4/pat_serial_tb.diag"; 
       `endif
     ...
     while (cur_pat <= last_pat && end_pat != 1) begin 
       ...
       case (opcode)
         ...
         'd`iddq_captureStmt: begin: iddq_capture_BLOCK
           cur_stmt = `iddq_captureStmt;
           set_args(memel[22:16]);
           iddq_capture(sid_args, sval_args);
         end 
   ...
   initial begin: TM4_occ_bypass_pattern__init_unit
     ...
     `ifdef tmax_vcde
       $display("XTB: Dumping Extended VCD information in file \"./max_sim/serial/occ_bypass/iddq/TM4/pat_serial_tb.vcd\"");
       $dumpports(`XTB_DUT_HIER, "./max_sim/serial/occ_bypass/iddq/TM4/pat_serial_tb.vcd");
     `endif
   ...
   `ifdef tmax_iddq
     $display("XTB: IDDQ test mode enabled");
     if (verbose >= 1) $display("XTB: Initializing IDDQ PLI (time %0t)", $time);
     $ssi_iddq("verb on");
     $ssi_iddq("cycle 0");
     $ssi_iddq("dut async_fifo_test.dut");

     `ifdef tmax_iddq_seed_mode
       iddq_sm =`tmax_iddq_seed_mode;
       if (iddq_sm < 0 || iddq_sm > 2)  begin 
         $display("XTB Error: wrong seed mode (tmax_iddq_seed_mode = %0d), should be 0 (automatic fault seeding), 1 (seeding from file) or 2 (both seeding mode simulatnesouly). Exiting... ",`tmax_iddq_seed_mode);
         $finish;
       end
     `else
       iddq_sm = 0;
     `endif
     if (iddq_sm == 0 || iddq_sm == 2) begin 
       if (verbose >= 1)  $display("XTB: IDDQ test using automatic fault seeding");
       $ssi_iddq("seed SA async_fifo_test.dut");
     end
     if (iddq_sm == 1 || iddq_sm == 2) begin 
       if (verbose >= 1)  $display("XTB: IDDQ test using fault seeding from file async_fifo_test.faults");
       seedf = $fopen("async_fifo_test.faults", "r");
       if (seedf == 0) begin
         $display("XTB Error: Unable to open file async_fifo_test.faults (for fault seeding). Exiting...\n");
         $finish; 
       end 
       else $fclose(seedf);
       $ssi_iddq("scope async_fifo_test.dut");
       $ssi_iddq("read_tmax async_fifo_test.faults");
     end
   `endif
